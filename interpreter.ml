...
let exec_prog (p: program): unit =
  ...
  List.iter (fun o -> 
   List.iter (fun (x, t, _, _) -> 
      match t with
      | TArray(_, len) -> Hashtbl.add env (o.class_name ^ "." ^ x) (VArray (Array.make len Null))
  ...
  let rec eval_call f this args =
    ...
    let () = List.iter (fun (x, t, _, _) ->
                match t with
                | TArray(_, len) -> Hashtbl.add call_env x (VArray (Array.make len Null))
  ...
  and exec_seq s lenv =
    ...
    and eval (e: expr): value = match e with
      ...
      | New(i)             -> (try let o = Hashtbl.find cls i 
                                    in match o.parent with
                                        | None -> let envo = Hashtbl.create 16  (* Cas sans parent *)
                                                  in List.iter (fun (x, t, _, s) -> match s, t with
                                                                                    | None, TArray (_, len) -> Hashtbl.add envo x (VArray (Array.make len Null))
      ...
      | Super _             -> let ob = Hashtbl.find lenv "this" in
                              let o = (match ob with VObj o -> o | _ -> error "Not an object") in
