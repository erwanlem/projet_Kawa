open Kawa
open Typechecker

type value =
  | VInt  of int
  | VBool of bool
  | VObj  of obj
  | Null
and obj = {
  cls:    string;
  fields: (string, value) Hashtbl.t;
}

exception Error of string
exception Return of value

let exec_prog (p: program): unit =
  let env = Hashtbl.create 16 in
  List.iter (fun (x, _) -> Hashtbl.add env x Null) p.globals;
  let cls = Hashtbl.create 16 in
  List.iter (fun o -> Hashtbl.add cls o.class_name o) p.classes;

  
  let rec eval_call f this args =
    let call_env = Hashtbl.create 16 in
    let () = (Hashtbl.add call_env "this" (VObj this) ) in
    let () = List.iter (fun (x, _) -> Hashtbl.add call_env x Null) f.locals;
    let rec set_args arg_name arg =
      match arg_name, arg with
      | [], [] -> ()
      | (n, t) :: l, v :: l' -> Hashtbl.add call_env n v; set_args l l'
      | _, _ -> ()
    in set_args f.params args
    in exec_seq f.code call_env


  and exec_seq s lenv =
    let rec evali e = match eval e with
      | VInt n -> n
      | VBool b -> if b = true then 1 else 0
      | _ -> assert false
    and evalb e = match eval e with
      | VBool b -> b
      | _ -> assert false
    and evalo e = match eval e with
      | VObj o -> o
      | _ -> assert false
    
        
    and eval (e: expr): value = match e with
      | Int n  -> VInt n
      | Bool b -> VBool b
      | Binop(Add, e1, e2) -> VInt(evali e1 + evali e2)
      | Binop(Sub, e1, e2) -> VInt(evali e1 - evali e2)
      | Binop(Mul, e1, e2) -> VInt(evali e1 * evali e2)
      | Binop(Div, e1, e2) -> VInt(evali e1 / evali e2)
      | Binop(Rem, e1, e2) -> VInt(evali e1 mod evali e2)
      | Binop(Lt , e1, e2) -> VBool(evali e1 < evali e2)
      | Binop(Le , e1, e2) -> VBool(evali e1 <= evali e2)
      | Binop(Gt , e1, e2) -> VBool(evali e1 > evali e2)
      | Binop(Ge , e1, e2) -> VBool(evali e1 >= evali e2)
      | Binop(Neq, e1, e2) -> VBool(evali e1 <> evali e2)
      | Binop(Eq, e1, e2)  -> VBool(evali e1 = evali e2)
      | Binop(And, e1, e2) -> VBool(evalb e1 && evalb e2)
      | Binop(Or, e1, e2)  -> VBool(evalb e1 || evalb e2)
      | Unop(Opp, e)       -> VInt(- evali e)
      | Unop(Not, e)       -> VBool(Bool.not (evalb e))
      | New(i)             -> (try let o = Hashtbl.find cls i 
                                    in match o.parent with
                                        | None -> let envo = Hashtbl.create 16  (* Cas sans parent *)
                                                  in List.iter (fun (x, _) -> Hashtbl.add envo x Null) o.attributes; 
                                                  VObj({cls=i; fields=envo})
                                        | Some p -> let parent = evalo (New p) (* Cas avec parent *)
                                                  in let envo = parent.fields 
                                                  in List.iter (fun (x, _) -> Hashtbl.add envo x Null) o.attributes; 
                                                  VObj({cls=i; fields=envo})
                                with Not_found -> raise (Error "Invalid class name") )
      | Get(m)          -> (match m with
                            | Var v -> (try Hashtbl.find lenv v
                                        with Not_found -> ( Hashtbl.find env v) )
                            | Field(e', i) -> let o = evalo e' 
                                              in (try Hashtbl.find o.fields i 
                                                  with Not_found -> raise (Error "Invalid attribute name") ) )
      | This                -> (try let v = Hashtbl.find lenv "this" in v with Not_found -> raise (Error "Invalid this") ) 
      | MethCall(e1, i, l)  ->  let o = evalo e1
                                in let obj_class = Hashtbl.find cls o.cls
                                in let methods = obj_class.methods
                                in (* Recherche la méthode correspondante et l'appelle *)
                                let rec find_mtd m c = 
                                  (try List.find (fun a -> a.method_name = i) m
                                  with Not_found -> (match c.parent with
                                                    | None -> raise (Error "Method not found")
                                                    | Some p -> find_mtd m (Hashtbl.find cls p)
                                                    ) )
                                in let mtd = find_mtd methods obj_class
                                in let l = List.map eval l in
                                (try let () = eval_call mtd o l in Null with Return v -> v) (* Appel *)

      | NewCstr(i, l)       ->  let o = evalo (New i)
                                in let obj_class = Hashtbl.find cls o.cls
                                in let methods = obj_class.methods
                                in (* Appel du constructeur avec les paramètres donnés *)
                                let constr =  List.find (fun a -> a.method_name = "constructor") methods
                                in let l = List.map eval l
                                in (try let () = eval_call constr o l in VObj(o) 
                                    with Return v -> raise (Error "Invalid construct return value"))
    in
  
    let rec exec (i: instr): unit = match i with
      | Print e           -> (match eval e with
                            | VInt v  -> Printf.printf "%d\n%!" v
                            | VBool b -> Printf.printf "%b\n%!" b
                            | Null    -> Printf.printf "Null\n%!"
                            | _       -> Printf.printf "Obj\n%!" )
      | If(e, i1, i2)     -> if evalb e then exec_seq i1 else exec_seq i2
      | Expr(e)           -> let _ = eval e in ()
      | Set(m, e)         -> (match m with
                              | Var v        -> (try let _ = Hashtbl.find lenv v in Hashtbl.replace lenv v (eval e) 
                                                with Not_found -> let _ = Hashtbl.find env v in Hashtbl.replace env v (eval e) )
                              | Field(e', i) -> let o = evalo e' 
                                                in (
                                                try let _ = Hashtbl.find o.fields i in Hashtbl.replace o.fields i (eval e) 
                                                with Not_found -> raise (Error "Invalid attribute name") ) )
      | While(e, s) as i  -> if evalb e then (exec_seq s; exec i)
      | Return(e)         -> raise (Return (eval e) )
    and exec_seq s = 
      List.iter exec s
    in

    exec_seq s
  in
  exec_seq p.main (Hashtbl.create 1)
